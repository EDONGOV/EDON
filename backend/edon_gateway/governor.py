...
class EDONGovernor:
    ...
    def __init__(self, policy_config: Optional[PolicyConfig] = None, db=None):
        """Initialize governor."""
        self.policy_engine = PolicyEngine(policy_config)  # type: ignore[arg-type]
        self.db = db
    ...
    def _evaluate_impl(
        ...
        if computed_risk in (self.policy_engine.config.escalate_risk_levels or set()):
    ...
    def _check_intent_alignment(self, action: Action, intent: IntentContract) -> bool:
        ...
        scope_lower = " ".join(intent.scope.keys()).lower()
        ...
        action_keywords = {
            ...
            # Catch-all (empty list — handled by tool_str not in dict logic below)
            Tool.CUSTOM: [],
        }

        # If tool is not in action_keywords at all, allow (custom/unknown tools)
