...
def _select_security_model(findings: list[dict[str, Any]]) -> str:
    severities = {f["severity"] for f in findings}
    types      = {f["type"] for f in findings}

    if "CRITICAL" in severities:
        return _MODEL_SONNET   # prompt injection, credential abuse — need real reasoning
    if "prompt_injection" in types or "bypass_probe" in types:
        return _MODEL_SONNET   # adversarial patterns always warrant Sonnet
    if len(findings) >= 3 or "HIGH" in severities:
        return _MODEL_SONNET   # multiple findings or any HIGH — Sonnet
    return _MODEL_HAIKU        # MEDIUM/LOW recon or escalation blips — Haiku is fine
...
def analyse_threats(findings: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    model  = _select_security_model(findings)
    print(f"[secmon] Using {model} for threat analysis ({len(findings)} finding(s))")
...
def _open_issue(title: str, body: str, severity: str = "UNKNOWN") -> str | None:
    if not GITHUB_TOKEN:
        return None
    issue_decision = gov_check(
        agent_id="security_monitor_agent",
        action_type="github.issue_create",
        parameters={"repo": GITHUB_REPO, "severity": severity},
        stated_intent=f"file {severity} security alert for detected threat pattern",
    )
...
*Auto-generated by EDON Security Monitor — {datetime.now(UTC).isoformat()}*"""
    ...
    issue_url = _open_issue(title, issue_body, severity=sev)
    if issue_url:
    ...
    if WEBHOOK_URL and analysis.get("escalate_immediately"):
        webhook_decision = gov_check(
            ...
            parameters={"severity": sev, "finding_count": len(findings)},
            stated_intent=f"escalate {sev} security alert immediately via webhook",
        )
        if webhook_decision:
